...
pw.Expanded(
                    ...
                    pw.Container(
                      child: pw.Column(
                        ...
                        children: [
                          ...
                          if (estimate.discountType != 'none')
                            pw.Padding(
                              padding: const pw.EdgeInsets.symmetric(horizontal: 6, vertical: 4),
                              child: pw.Row(
                                mainAxisAlignment: pw.MainAxisAlignment.spaceBetween,
                                children: [
                                  pw.Text('Discount (${estimate.discountType})', style: const pw.TextStyle(fontSize: 8.5)),
                                  pw.Text('- ₹ ${estimate.discountAmount?.toStringAsFixed(2) ?? ''}', style: pw.TextStyle(fontFallback: [rupeeFont], fontSize: 8.5)),
                                ],
                              ),
                            ),
                          if (estimate.discountType != 'none')
                            pw.Padding(
                              ...
                              child: pw.Row(
                                mainAxisAlignment: pw.MainAxisAlignment.spaceBetween,
                                children: [
                                  pw.Text('Taxable Value', style: const pw.TextStyle(fontSize: 8.5)),
                                  pw.Text('₹ ${estimate.taxableAmount?.toStringAsFixed(2) ?? ''}', style: pw.TextStyle(fontFallback: [rupeeFont], fontSize: 8.5)),
                                ],
                              ),
                            ),
                          pw.Container(
                      ...
                      child: pw.Column(
                        ...
                        children: [
                          ...
                          pw.Padding(
                            ...
                            child: pw.Row(
                              children: [
                                pw.Expanded(child: pw.Text('Sub Total', style: const pw.TextStyle(fontSize: 8.5))),
                                pw.Text(
                                  '₹ ${((invoice.subtotal != null && invoice.subtotal! > 0) ? invoice.subtotal! : invoice.items.fold<double>(0.0, (s, e) => s + ((e.amount != null && e.amount! > 0) ? e.amount! : (e.pricePerUnit * e.quantity)))).toStringAsFixed(2)}',
                          ...
                          if (invoice.discountType != 'none')
                            pw.Padding(
                              ...
                              child: pw.Row(
                                children: [
                                  pw.Expanded(child: pw.Text('Discount (${invoice.discountType})', style: const pw.TextStyle(fontSize: 8.5))),
                                  pw.Text('- ₹ ${invoice.discountAmount?.toStringAsFixed(2) ?? '0.00'}', style: pw.TextStyle(fontFallback: [rupeeFont], fontSize: 8.5)),
                                ],
                              ),
                            ),
                          if (invoice.discountType != 'none')
                            pw.Padding(
                              ...
                              child: pw.Row(
                                children: [
                                  pw.Expanded(child: pw.Text('Taxable Value', style: const pw.TextStyle(fontSize: 8.5))),
                                  pw.Text('₹ ${invoice.taxableAmount?.toStringAsFixed(2) ?? '0.00'}', style: pw.TextStyle(fontFallback: [rupeeFont], fontSize: 8.5)),
                                ],
                              ),
                            ),
                          pw.Padding(
                          ...
                          pw.Container(
                            ...
                            child: pw.Row(
                              children: [
                                pw.Expanded(child: pw.Text('Grand Total', style: pw.TextStyle(fontWeight: pw.FontWeight.bold, fontSize: 9))),
                                pw.Text('₹ ${invoice.totalAmount?.toStringAsFixed(2) ?? '0.00'}', style: pw.TextStyle(fontFallback: [rupeeFont], fontWeight: pw.FontWeight.bold, fontSize: 9)),
                              ],
                            ),
                          ),
                        ],
    ...
    pdf.addPage(
      pw.MultiPage(
        ...
        build: (pw.Context context) {
          ...
          for (int i = 0; i < invoice.items.length; i++) {
            final item = invoice.items[i];
            final itemAmt = (item.amount != null && item.amount! > 0)
                ? item.amount!
            ...
            itemsRows.add(pw.TableRow(
              children: [
                ...
                pw.Padding(padding: const pw.EdgeInsets.all(4), child: pw.Text('${item.quantity}', textAlign: pw.TextAlign.right, style: const pw.TextStyle(fontSize: 8.5))),
                pw.Padding(padding: const pw.EdgeInsets.all(4), child: pw.Text('₹ ${item.pricePerUnit.toStringAsFixed(2)}', textAlign: pw.TextAlign.right, style: pw.TextStyle(fontFallback: [rupeeFont], fontSize: 8.5))),
                pw.Padding(padding: const pw.EdgeInsets.all(4), child: pw.Text('₹ ${itemAmt.toStringAsFixed(2)}', textAlign: pw.TextAlign.right, style: pw.TextStyle(fontFallback: [rupeeFont], fontSize: 8.5))),
              ]
            ));
          }
          ...
          itemsRows.add(pw.TableRow(
            children: [
              ...
              pw.Padding(padding: const pw.EdgeInsets.all(4), child: pw.Text('${invoice.items.fold<double>(0, (p, e) => p + e.quantity).toInt()}', textAlign: pw.TextAlign.right, style: pw.TextStyle(fontWeight: pw.FontWeight.bold, fontSize: 9))),
              pw.Padding(padding: const pw.EdgeInsets.all(4), child: pw.Text('')),
              pw.Padding(padding: const pw.EdgeInsets.all(4), child: pw.Text('₹ ${invoice.totalAmount?.toStringAsFixed(2) ?? '0.00'}', textAlign: pw.TextAlign.right, style: pw.TextStyle(fontFallback: [rupeeFont], fontWeight: pw.FontWeight.bold, fontSize: 9))),
            ]
          ));
                      ...
                      child: pw.Column(
                        ...
                        children: [
                          ...
                          pw.Container(
                            ...
                            child: pw.Row(
                              children: [
                                pw.Expanded(child: pw.Text('Received Amount', style: pw.TextStyle(fontSize: 8.5))),
                          ...
                          pw.Container(
                            ...
                            child: pw.Row(
                              children: [
                                pw.Expanded(child: pw.Text('Balance Remaining', style: pw.TextStyle(fontWeight: pw.FontWeight.bold, fontSize: 9))),
  ...
  Future<Uint8List> generateMergedBillingInvoicesPdf(List<BillingInvoiceModel> invoices) async {
    final rupeeFont = pw.Font.ttf(await rootBundle.load('assets/fonts/roboto1.ttf'));
    final pdf = pw.Document();
    ...
    // Page 1: Consolidated Cash Invoices Summary Statement
    final totalAmountSum = invoices.fold<double>(0.0, (s, i) => s + (i.totalAmount ?? 0));
    final totalReceivedSum = invoices.fold<double>(0.0, (s, i) => s + i.receivedAmount);
    final totalOutstandingSum = invoices.fold<double>(0.0, (s, i) => s + i.outstandingAmount);
          ...
          return pw.Column(
            ...
            children: [
              pw.Row(
                ...
                children: [
                  if (logoImage != null)
                    pw.Container(width: 140, height: 45, child: pw.Image(logoImage, fit: pw.BoxFit.contain))
                  else
                    pw.Text('DTS', style: pw.TextStyle(fontSize: 18, fontWeight: pw.FontWeight.bold, color: PdfColor.fromHex('#0B2545'))),
                  pw.Column(
                    crossAxisAlignment: pw.CrossAxisAlignment.end,
                    children: [
                      pw.Text('DIESEL TECHNICAL SOLUTIONS', style: pw.TextStyle(fontSize: 11, fontWeight: pw.FontWeight.bold, color: PdfColor.fromHex('#0B2545'))),
                      pw.Text('CASH INVOICES STATEMENT / REPORT', style: pw.TextStyle(fontSize: 8.5, fontWeight: pw.FontWeight.bold, color: PdfColor.fromHex('#134074'))),
                      pw.Text('Generated: ${DateFormat('dd-MM-yyyy HH:mm').format(DateTime.now())}', style: const pw.TextStyle(fontSize: 7.5, color: PdfColors.grey700)),
                    ],
                  ),
                ],
              ),
              ...
              pw.Container(
                ...
                decoration: pw.BoxDecoration(
                  ...
                  border: pw.Border.all(color: PdfColor.fromHex('#0B2545'), width: 0.5),
                ),
                child: pw.Row(
                  mainAxisAlignment: pw.MainAxisAlignment.spaceBetween,
                  children: [
                    pw.Text('Total Invoices: ${invoices.length}', style: pw.TextStyle(fontWeight: pw.FontWeight.bold, fontSize: 9.5, color: PdfColor.fromHex('#0B2545'))),
                    pw.Text('Received: RS ${totalReceivedSum.toStringAsFixed(2)}', style: pw.TextStyle(fontWeight: pw.FontWeight.bold, fontSize: 9.5, color: PdfColors.green800)),
                    pw.Text('Outstanding: RS ${totalOutstandingSum.toStringAsFixed(2)}', style: pw.TextStyle(fontWeight: pw.FontWeight.bold, fontSize: 9.5, color: PdfColors.red800)),
                    pw.Text('Total: RS ${totalAmountSum.toStringAsFixed(2)}', style: pw.TextStyle(fontWeight: pw.FontWeight.bold, fontSize: 10, color: PdfColor.fromHex('#0B2545'))),
                  ],
                ),
              ),
              pw.SizedBox(height: 14),
              pw.Text('Invoices Summary Breakdown', style: pw.TextStyle(fontSize: 11, fontWeight: pw.FontWeight.bold, color: PdfColor.fromHex('#0B2545'))),
              pw.SizedBox(height: 6),
              pw.Table(
                border: pw.TableBorder.all(color: PdfColors.grey300, width: 0.5),
                columnWidths: const {
                ...
                children: [
                  pw.TableRow(
                    decoration: pw.BoxDecoration(color: PdfColor.fromHex('#0B2545')),
                    children: [
                      pw.Padding(padding: const pw.EdgeInsets.all(5), child: pw.Text('#', style: pw.TextStyle(color: PdfColors.white, fontWeight: pw.FontWeight.bold, fontSize: 8.5))),
                      pw.Padding(padding: const pw.EdgeInsets.all(5), child: pw.Text('Customer Name', style: pw.TextStyle(color: PdfColors.white, fontWeight: pw.FontWeight.bold, fontSize: 8.5))),
                      pw.Padding(padding: const pw.EdgeInsets.all(5), child: pw.Text('Invoice No.', style: pw.TextStyle(color: PdfColors.white, fontWeight: pw.FontWeight.bold, fontSize: 8.5))),
                      pw.Padding(padding: const pw.EdgeInsets.all(5), child: pw.Text('Date', style: pw.TextStyle(color: PdfColors.white, fontWeight: pw.FontWeight.bold, fontSize: 8.5))),
                      pw.Padding(padding: const pw.EdgeInsets.all(5), child: pw.Text('Status', style: pw.TextStyle(color: PdfColors.white, fontWeight: pw.FontWeight.bold, fontSize: 8.5))),
                      pw.Padding(padding: const pw.EdgeInsets.all(5), child: pw.Text('Total Amount', style: pw.TextStyle(color: PdfColors.white, fontWeight: pw.FontWeight.bold, fontSize: 8.5))),
                    ],
                  ),
                  ...List.generate(invoices.length, (idx) {
                    final inv = invoices[idx];
                    final invNo = inv.invoiceNumber ?? '-';
                    final status = inv.paymentStatus.replaceAll('_', ' ').toUpperCase();
                    return pw.TableRow(
                      decoration: pw.BoxDecoration(color: idx % 2 == 0 ? PdfColors.white : PdfColor.fromHex('#F9FAFB')),
                      children: [
                        pw.Padding(padding: const pw.EdgeInsets.all(5), child: pw.Text('${idx + 1}', style: const pw.TextStyle(fontSize: 8))),
                        pw.Padding(padding: const pw.EdgeInsets.all(5), child: pw.Text(inv.billTo.customerName, style: pw.TextStyle(fontSize: 8, fontWeight: pw.FontWeight.bold))),
                        pw.Padding(padding: const pw.EdgeInsets.all(5), child: pw.Text(invNo, style: const pw.TextStyle(fontSize: 8))),
                        pw.Padding(padding: const pw.EdgeInsets.all(5), child: pw.Text(DateFormat('dd-MM-yyyy').format(inv.invoiceDate), style: const pw.TextStyle(fontSize: 8))),
                        pw.Padding(padding: const pw.EdgeInsets.all(5), child: pw.Text(status, style: pw.TextStyle(fontSize: 7.5, fontWeight: pw.FontWeight.bold, color: status == 'PAID' ? PdfColors.green700 : (status == 'PARTIALLY PAID' ? PdfColors.orange700 : PdfColors.red700)))),
                        pw.Padding(padding: const pw.EdgeInsets.all(5), child: pw.Text('RS ${(inv.totalAmount ?? 0).toStringAsFixed(2)}', style: pw.TextStyle(fontSize: 8, fontWeight: pw.FontWeight.bold))),
                      ],
                    );
                  }),
                ],
    ...
    for (final invoice in invoices) {
      ...
      pdf.addPage(
        pw.MultiPage(
          ...
          build: (pw.Context context) {
            return [
              _buildEstimateHeader(logoImage, 'CASH INVOICE', null, null),
              pw.SizedBox(height: 10),
              pw.Table(
                border: pw.TableBorder.all(color: PdfColors.grey300, width: 0.5),
                children: [
                    ...
                    children: [
                      ...
                      pw.Padding(
                        ...
                        child: pw.Column(
                          crossAxisAlignment: pw.CrossAxisAlignment.start,
                          children: [
                            pw.Text('Invoice No: ${invoice.invoiceNumber ?? '-'}', style: pw.TextStyle(fontWeight: pw.FontWeight.bold, fontSize: 9)),
                            pw.Text('Date: ${DateFormat('dd-MM-yyyy').format(invoice.invoiceDate)}', style: const pw.TextStyle(fontSize: 8.5)),
                            pw.Text('Status: ${invoice.paymentStatus.toUpperCase()}', style: pw.TextStyle(fontWeight: pw.FontWeight.bold, fontSize: 8.5, color: invoice.paymentStatus.toLowerCase() == 'paid' ? PdfColors.green700 : PdfColors.orange700)),
                          ],
                        ),
                      ),
                    ],
              ...
              pw.Row(
                mainAxisAlignment: pw.MainAxisAlignment.spaceBetween,
                children: [
                  pw.Text('Total Amount: RS ${(invoice.totalAmount ?? 0).toStringAsFixed(2)}', style: pw.TextStyle(fontWeight: pw.FontWeight.bold, fontSize: 10)),
                  pw.Text('Received: RS ${invoice.receivedAmount.toStringAsFixed(2)} | Outstanding: RS ${invoice.outstandingAmount.toStringAsFixed(2)}', style: pw.TextStyle(fontSize: 9, fontWeight: pw.FontWeight.bold)),
                ],
              ),
            ];
          },
  ...
  Future<void> printOrSaveMergedBillingInvoicesPdf(List<BillingInvoiceModel> invoices, String filename) async {
    final bytes = await generateMergedBillingInvoicesPdf(invoices);
    await Printing.layoutPdf(
      onLayout: (PdfPageFormat format) async => bytes,
      name: filename,
    );
  ...
  Future<void> shareMergedBillingInvoicesPdf(List<BillingInvoiceModel> invoices, String filename) async {
    final bytes = await generateMergedBillingInvoicesPdf(invoices);
    final tempDir = await getTemporaryDirectory();
  ...
  Future<Uint8List> generateMergedTaxInvoicesPdf(List<TaxInvoiceModel> invoices) async {
    final rupeeFont = pw.Font.ttf(await rootBundle.load('assets/fonts/roboto1.ttf'));
    final pdf = pw.Document();
        ...
        pw.MultiPage(
          ...
          build: (pw.Context context) {
            return [
              ...
              // Footer (Bank details cell left empty | Terms | Signature)
              pw.Table(
                border: pw.TableBorder.all(color: PdfColors.black, width: 0.5),
                columnWidths: const {
                  0: pw.FlexColumnWidth(3.5),
  ...
  Future<void> printOrSaveMergedTaxInvoicesPdf(List<TaxInvoiceModel> invoices, String filename) async {
    final bytes = await generateMergedTaxInvoicesPdf(invoices);
    await Printing.layoutPdf(
      onLayout: (PdfPageFormat format) async => bytes,
      name: filename,
    );
  ...
  Future<void> shareMergedTaxInvoicesPdf(List<TaxInvoiceModel> invoices, String filename) async {
    final bytes = await generateMergedTaxInvoicesPdf(invoices);
    final tempDir = await getTemporaryDirectory();
    final file = File('${tempDir.path}/$filename.pdf');
    await file.writeAsBytes(bytes);
